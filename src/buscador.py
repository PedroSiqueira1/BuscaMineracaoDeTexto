...
def buscador():
    ...
    try:
        ...
        use_stemmer = config['STEMMER'][0]
        if use_stemmer == 'STEMMER':
            stemmer = PorterStemmer()
            logger.info("Using stemmer to search queries")
        ...
        if use_stemmer == 'STEMMER':
            for i in range(len(query_tokens)):
                query_tokens[i] = [stemmer.stem(word).upper() for word in query_tokens[i]]
        ...
        if use_stemmer == 'STEMMER':
            df.to_csv('./results/' + query_documents[0][0:-4] + "-STEMMER.csv", sep=';', index=False)
        else:
            df.to_csv('./results/' + query_documents[0][0:-4] + "-NOSTEMMER.csv", sep=';', index=False)
